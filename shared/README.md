...
- `commonMain/transfer` 的 `EasyOpenTransferProfile` 将 `androidMac` 定义为可选硬件匹配字段；它来自开门器 Manufacturer Data 中镜像的 MAC，iOS 不写入 `CBPeripheral.identifier`。Android/iOS 导入会优先匹配广播身份，匹配不到时保留手动选择兜底，并兼容旧 `address` 字段。`EasyOpenQrCodec` 提供 Android/iOS 共用的 `EASYOPEN-SHARE:3:` 加密二维码载荷；无硬件绑定的导入配置会进入重新扫描/绑定流程。

尚未把所有 Android 页面、存储、NFC、相机和配对流程迁移到 shared；这些迁移必须逐批编译并保留 Android 回滚路径。协议核心、配对页面、iOS 二维码和 Core NFC 适配已经完成首批迁移。
